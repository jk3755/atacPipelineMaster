...
## Timeout path - file to be generated to indicate this thread timed out
timeoutPath <- paste0(dirPath, "footprints/operations/raw/timeout", sampleName, ".", geneName, ".rawFootprintData.timeout")
...
##
analyzeRawFootprint <- function(numMotif){
...
} # end analyzeRawFootprint function
...
tryCatch(
  expr = {
    withTimeout(analyzeRawFootprint(numMotif),
                timeout = 10)
  },
  TimeoutException = function(ex) {cat("Timeout. Skipping for now\n"); file.create(timeoutPath)}
)

## Save the raw footprint data
cat("Saving finished data for", geneName, "\n")

tryCatch(save(footprintData, file = footprintDataPath))
