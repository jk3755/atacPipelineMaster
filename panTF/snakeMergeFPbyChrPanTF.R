...
for (x in 1:num_motifs){
  ...
  signalpath <- paste0(dirpath, "pantf/data/merged/", sample, ".", gene, ".", "motif", x, ".merged.Rdata")
  ...
  if (file.exists(signalpath) == TRUE){
    ...
    for (b in chr_names){
      ...
      com <- paste0(b, "_in <- gsub('", b, ".pantf.done.bychr.txt', paste0('motif', x, '.", b ,".Rdata'), ", b, "_input)")
      eval(parse(text = com))
