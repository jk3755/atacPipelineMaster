...
for (x in 1:num_motifs){
  ...
  if (file.exists(signalpath) == TRUE){
    ...
    for (b in chr_names){
      ...
      com <- paste0(b, "_in <- gsub('", b, ".footprint.bychr.sat.done.txt', paste0('motif', x, '.", b ,".Rdata'), ", b, "_input)")
      eval(parse(text = com))
      
      com <- paste0(b, "_in <- gsub('operations/footsat', 'saturation/footprints/data/bychr', ", b, "_in)")
      eval(parse(text = com))
