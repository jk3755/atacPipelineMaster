...
##
cat("Parsing binding sites for gene", genename, " from sample", samplename, "downsampled at", sampleProb, "\n")
load(sitespath)
...
for (x in 1:num_motifs){
  ...
  if (file.exists(info_path) == TRUE){
    ...
    ##
    cat("Loading data...", "\n")
    mergein <- gsub("done.merged.txt", paste0("motif", x, ".merged.Rdata"), mergedpath)
    mergein <- gsub("operations/footsat", "saturation/footprints/data/merged", mergein)
    ...
    ## Make graph
    svg_path <- paste0(dirpath, "saturation/footprints/graphs/", samplename, ".", sampleProb, ".", genename, ".", "motif", x, ".peak.sites.svg")
    ...
    func <- tryCatch({
      ...
      svg_path <- paste0(dirpath, "saturation/footprints/graphs/", samplename, ".", sampleProb, ".", genename, ".", "motif", x, ".bfpeak.sites.svg")
      svg(file = svg_path) # set the filepath for saving the svg figure
