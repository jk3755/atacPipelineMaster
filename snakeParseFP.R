...
for (x in 1:num_motifs){
  ...
  if (file.exists(info_path) == TRUE){
    ...
    ## Make graph
    svg_path <- paste0(dirpath, "footprints/parsed/", samplename, ".", genename, ".", "motif", x, ".peak.sites.svg")
    ...
    ## Make graph
    svg_path <- paste0(dirpath, "footprints/parsed/", samplename, ".", genename, ".", "motif", x, ".bfpeak.sites.svg")
    svg(file = svg_path) # set the filepath for saving the svg figure
    cat("Saving svg footprint image at path:", svg_path, "\n")
    PWMin <- pwm2pfm(PWM)
